...
import {fetchAssessment_Reading_Popup_dimension} from "../../redux/slices/Indicatorpopup"
import LoaderContainer from "../LoaderContainer";

export default function CultureofExcellenceKeyIndicators(props) {
...
/* redux state */
const Assessment_Reading_Popup_dimensionloading = useSelector(state => state.indicatorpopup.Assessment_Reading_Popup_dimensionloading)
const Assessment_Reading_Popup_dimension = useSelector(state => state.indicatorpopup.Assessment_Reading_Popup_dimension)
/* */
...
useEffect(()=>{
  if(selectedDimension && visible){
    dispatch(fetchAssessment_Reading_Popup_dimension({
      "elasticQueryName": "",
...
const ChartData = useMemo(()=>{
  ...
  let data = Assessment_Reading_Popup_dimension
  ?.filter(item => item[selectedDimension.code])
  .reduce((acc,item)=>{
    let {KPS_AVERAGE, PERFORMANCE_RELATIVE, [selectedDimension.code]:Dimension} = item
    // acc.push({value: [KPS_AVERAGE,PERFORMANCE_RELATIVE,4000,Dimension,Dimension], name: Dimension})
          ...
          <div>
            ...
            <div className="brand-neutral-50 flex items-center 2xl:gap-[0.833vw] gap-4   2xl:h-[3.125vw] h-[65px] xl:h-[55px]">
              ...
              <div className=" text-[16px] xl:text-[0.885vw] font-normal leading-6 2xl:leading-[1.219vw] flex items-center 2xl:gap-[0.417vw] gap-1">
                ...
                <div className="white_text_color 2xl:text-[0.938vw] text-base font-normal leading-6 2xl:leading-[1.25vw]">
                  <p> i-Ready Benchmark Assessment - Reading</p>
                </div>
              </div>
            </div>
            <ScrollPanel
            ...
            >
              <div className="bg_blue_color_900  p-[18px] xl:p-[18px] 3xl:p-[0.938vw] 2xl:space-y-[1.25vw] space-y-[20px]">
                ...
                <div>
                  <ChartWrapper
                    formatDownloadedData={Assessment_Reading_Popup_dimension}
                    formatFileName={"i-Ready Benchmark Assessment - Reading"}
                    ExportIcon={true}
                    titleshow={true}
                        ...
                        <div>
                          ...
                          <div>
                            <div className="h-[400px] xl:h-[20.323vw]">
                              <LoaderContainer loading={Assessment_Reading_Popup_dimensionloading}>
                                <BubbleChart
                                data={ChartData.data}
