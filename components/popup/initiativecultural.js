...
import { fetchCulture_Initiative_popup_dimension_1, fetchCulture_Initiative_popup_dimension_2,fetchCulture_Initiative_popup_dimension_3,fetchCulture_Initiative_popup_dimension_4,
  fetchCulture_Excellence_Plan_of_Action
} from "../../redux/slices/popup";
import { FormatNum } from "../utils";
import LoaderContainer from "../LoaderContainer";

export default function InitiativeCultural(props) {
  ...
  const Dimensions = [
    // { name: 'All', code: 'NY' },
    // { name: 'Establish a structure to conduct ongoing curriculum audits and adopt evidence-based tools, resources and practices for academics and social-emotional learning', code: 'RM' },
    // { name: 'Develop coherent instructional scope and sequence and pacing guides to maximize student learning and engagement across all grades', code: 'LDN' },
    // { name: 'Adopt and train staff in a balanced assessment system.', code: 'IST' },
...
/* Redux States */
  const Culture_Initiative_popup_dimension_1loading = useSelector(state => state.popup.Culture_Initiative_popup_dimension_1loading)
  const Culture_Initiative_popup_dimension_1 = useSelector(state => state.popup.Culture_Initiative_popup_dimension_1)

  const Culture_Initiative_popup_dimension_2loading = useSelector(state => state.popup.Culture_Initiative_popup_dimension_2loading)
  const Culture_Initiative_popup_dimension_2 = useSelector(state => state.popup.Culture_Initiative_popup_dimension_2)

  const Culture_Initiative_popup_dimension_3loading = useSelector(state => state.popup.Culture_Initiative_popup_dimension_3loading)
  const Culture_Initiative_popup_dimension_3 = useSelector(state => state.popup.Culture_Initiative_popup_dimension_3)

  const Culture_Initiative_popup_dimension_4loading = useSelector(state => state.popup.Culture_Initiative_popup_dimension_4loading)
  const Culture_Initiative_popup_dimension_4 = useSelector(state => state.popup.Culture_Initiative_popup_dimension_4)

  const Culture_Excellence_Plan_of_Actionloading = useSelector(state => state.popup.Culture_Excellence_Plan_of_Actionloading)
  const Culture_Excellence_Plan_of_Action = useSelector(state => state.popup.Culture_Excellence_Plan_of_Action)
/*  */
...
useEffect(()=>{
  if(selectedDimension && visible){
    if (Tile == 1){
      dispatch(fetchCulture_Initiative_popup_dimension_1({
          "elasticQueryName": "",
            "filters": [],
            "dynamicColumns": [
              {
      ...
      dispatch(fetchCulture_Initiative_popup_dimension_2({
            "elasticQueryName": "",
              "filters": [],
              "dynamicColumns": [
                {
      ...
      dispatch(fetchCulture_Initiative_popup_dimension_3({
      "elasticQueryName": "",
        "filters": [],
        "dynamicColumns": [
          {
      ...
      dispatch(fetchCulture_Initiative_popup_dimension_4({
      "elasticQueryName": "",
        "filters": [],
        "dynamicColumns": [
          {
...
useEffect(()=>{
  if (visible){
    dispatch(fetchCulture_Excellence_Plan_of_Action({
      "elasticQueryName": "",
        "filters": [],
        "dynamicColumns": [],
    }))
...
useEffect(()=>{
  if(selectedDimension){
    if (Tile == 1){
      setactiveTileBarData(Culture_Initiative_popup_dimension_1)
    } else if (Tile == 2){
      setactiveTileBarData(Culture_Initiative_popup_dimension_2)
    } else if (Tile == 3){
      setactiveTileBarData(Culture_Initiative_popup_dimension_3)
    } else if (Tile == 4){
      setactiveTileBarData(Culture_Initiative_popup_dimension_4)
    }
  }
},[selectedDimension, Tile, Culture_Initiative_popup_dimension_1, Culture_Initiative_popup_dimension_2, Culture_Initiative_popup_dimension_3, Culture_Initiative_popup_dimension_4])
...
const events = useMemo(()=>{
  let array_len = Culture_Excellence_Plan_of_Action.length
  let Initiative = {}
  let data = Culture_Excellence_Plan_of_Action.reduce((acc,item,index)=>{
  ...
  for (let range in data){
    ...
    for (let date in data[range]){
      ...
      let value = {
          ...
          className: index == 0 ? "event-flagIconstart" : 
            Boards.length != 0 ?
            'event-flagIcon'
            : 'event-ordered'
    ...
    <>
      <div>
        <Dialog header="Header"
          ...
          onHide={onHide}>
          <div>
            <div className="bg_bg_brand_neutral_100 grid grid-cols-1 md:grid-cols-12 flex items-center justify-between 2xl:gap-[1.563vw] gap-[20px] 3xl:gap-[1.25vw] 2xl:py-[0.677vw] py-[13px] 2xl:px-[1.25vw] px-5">
              <div className="grid grid-cols-1 md:grid-cols-3 items-center col-span-4">
                <div className="col-span-1 md:col-span-1"><Image src={'/images/Logo.png'} width={102} height={70} alt="Equitable Opportunities & Outcome" className="2xl:w-[5.313vw] 2xl:h-[3.646vw]" /></div>
                <div className="2xl:w-[16.094vw] col-span-1 md:col-span-2">
                  {/* <div className="white_text_color text-xs 2xl:text-[0.625vw] font-light leading-3 2xl:leading-[0.833vw]">PREMIER TEACHING</div> */}
                  <div className="2xl:text-[0.938vw] text-sm white_text_color leading-5 2xl:leading-[1.146vw] font-bold"><span className="font-normal">CULTURE OF</span> EXCELLENCE</div>
                </div>
              </div>
              <div className="col-span-8 white_text_color text-xs opacity-60 2xl:text-[0.729vw] font-normal leading-4 2xl:leading-[0.885vw]">
                {/* <p>Each initiative represents the initiative KPS is tracking throughout the implementation of the strategic plan. Indicators correspond to one of the four strategic plan goals. For more informatoin regarding specific initiatives and action steps, please visit the Strategic plan Key Initiative Dashboard</p> */}
            ...
            <div className="brand-neutral-50 flex items-center 2xl:gap-[0.833vw] gap-4   2xl:h-[3.125vw] md:h-[65px] xl:h-[55px]">
              ...
              <div className="2xl:text-[0.938vw] font-normal leading-6 2xl:leading-[1.219vw] flex items-center 2xl:gap-[0.417vw] gap-1">
                ...
                {/* <div className="white_text_color 2xl:text-[0.938vw] text-base font-normal leading-6 2xl:leading-[1.25vw]"><p> Ensure safe, welcoming school communities</p></div> */}
              ...
              <div className="bg_blue_color_900  p-[18px] xl:p-[18px] 3xl:p-[0.938vw] 2xl:space-y-[1.25vw] space-y-[20px]">
                ...
                <div className="border border_color_interface-stroke-default 2xl:pt-[0.833vw] pt-3 2xl:px-[0.833vw]  px-3 2xl:space-y-[1.25vw] space-y-[20px] rounded-[0.417vw]">
                  ...
                  <div className="w-full overflow-auto">
                    <LoaderContainer loading={Culture_Excellence_Plan_of_Actionloading} width={"100%"} height={'10rem'}>
                ...
                <div className="grid grid-cols-1 2xl:gap-[1.25vw] gap-5">
                  ...
                  <div className="bg-[rgba(0,0,0,0.35)] 2xl:rounded-[0.417vw] rounded-xl 2xl:pt-[0.833vw] pt-3 2xl:px-[0.833vw]  px-3 2xl:space-y-[1.25vw] space-y-[22px]">
                    <div className="grid grid-cols-12 2xl:gap-[0.833vw] gap-3 bg-[#111928] rounded-[4px] pt-[0.833vw] px-[0.833vw]">
                    ...
                    <div className="h-[161px] xl:h-[16.385vw] max-md:w-[1000px]">
                ...
                {/* <div>
                  <ChartWrapper
                    ...
                    formatFileName={'Culture of Excellence Data'}
                          ...
                          <div className="bg-[#1F2A37] 2xl:rounded-[0.313vw] flex items-center rounded 2xl:p-[0.417vw] p-1.5 2xl:mt-[0.417vw] mt-1.5">
                            ...
                            <div>
                              <Dropdown value={selectedDimension} onChange={(e) => setselectedDimension(e.value)} options={Dimensions} optionLabel="name"
                                ...
                                placeholder="Select a Dimension" className="w-full md:w-[309px] 2xl:w-[18.229vw] 2xl:h-[1.51vw] h-[24px] custom_Dropdown" panelClassName="custom_Dropdownpanel custpanel" />
                            </div>
                          </div>
