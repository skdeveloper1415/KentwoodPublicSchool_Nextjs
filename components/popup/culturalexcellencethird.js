...
import {fetchState_Assessment_ELA_Popup_dimension} from "../../redux/slices/Indicatorpopup"
import LoaderContainer from "../LoaderContainer";
import { ChartSplitLine, FormatNum } from "../utils";

export default function CultureofExcellenceThird(props) {
...
/* redux state */
const State_Assessment_ELA_Popup_dimensionloading = useSelector(state => state.indicatorpopup.State_Assessment_ELA_Popup_dimensionloading)
const State_Assessment_ELA_Popup_dimension = useSelector(state => state.indicatorpopup.State_Assessment_ELA_Popup_dimension)
/* */
...
useEffect(()=>{
  if(selectedDimension  && visible){
    dispatch(fetchState_Assessment_ELA_Popup_dimension({
      "elasticQueryName": "",
...
const ChartData = useMemo(()=>{
  let SplitLine;
  let Labels = []
  let data = State_Assessment_ELA_Popup_dimension.reduce((acc,item)=>{
...
/*  */
  ...
  console.log("Excel Data",State_Assessment_ELA_Popup_dimension)
  const State_Assessment_ELA_Popup_dimensionExcelData = State_Assessment_ELA_Popup_dimension?.map((item) => {
    return {
          ...
          <div>
            ...
            <div className="brand-neutral-50 flex items-center 2xl:gap-[0.833vw] gap-4   2xl:h-[3.125vw] h-[65px] xl:h-[55px]">
              ...
              <div className="text-[16px] xl:text-[0.885vw] font-normal leading-6 2xl:leading-[1.219vw] flex items-center 2xl:gap-[0.417vw] gap-1">
                ...
                <div className="white_text_color 2xl:text-[0.938vw] text-base font-normal leading-6 2xl:leading-[1.25vw]">
                  <p> M-STEP State Assessment ELA</p>
                </div>
              </div>
            </div>
            <ScrollPanel
            ...
            >
              <div className="bg_blue_color_900  p-[18px] xl:p-[18px] 3xl:p-[0.938vw] 2xl:space-y-[1.25vw] space-y-[20px]">
                ...
                <div>
                  <ChartWrapper
                    formatDownloadedData={State_Assessment_ELA_Popup_dimensionExcelData}
                    formatFileName={"M-STEP State Assessment ELA"}
                    ExportIcon={true}
                    titleshow={true}
                    title={"M-STEP State Assessment English Language Arts"}
                    data={
                      <>
                        <div>
                          ...
                          <div className="h-[155px] xl:h-[21.185vw] 2xl:h-[17.385vw] 3xl:h-[15.385vw] max-md:w-[1000px]">
                                                        <LoaderContainer height={"100%"} width={'100%'} loading={State_Assessment_ELA_Popup_dimensionloading}>
                                                        <Barlinechart
                                                        legends={{
                                                            show:false,
                                                            left: 10,
