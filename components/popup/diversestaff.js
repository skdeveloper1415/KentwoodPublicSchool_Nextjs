...
import { fetchHighImpact_Initiative_popup_dimension_1, fetchHighImpact_Initiative_popup_dimension_2, fetchHighImpact_Initiative_popup_dimension_3,
  fetchHigh_Impact_Plan_of_Action
 } from "../../redux/slices/popup";
import { FormatNum } from "../utils";
import LoaderContainer from "../LoaderContainer";

export default function ImpactDiverseStaff(props) {
  ...
  /* Redux States */
  const HighImpact_Initiative_popup_dimension_1loading = useSelector(state => state.popup.HighImpact_Initiative_popup_dimension_1loading)
  const HighImpact_Initiative_popup_dimension_1 = useSelector(state => state.popup.HighImpact_Initiative_popup_dimension_1)

  const HighImpact_Initiative_popup_dimension_2loading = useSelector(state => state.popup.HighImpact_Initiative_popup_dimension_2loading)
  const HighImpact_Initiative_popup_dimension_2 = useSelector(state => state.popup.HighImpact_Initiative_popup_dimension_2)

  const HighImpact_Initiative_popup_dimension_3loading = useSelector(state => state.popup.HighImpact_Initiative_popup_dimension_3loading)
  const HighImpact_Initiative_popup_dimension_3 = useSelector(state => state.popup.HighImpact_Initiative_popup_dimension_3)

  const High_Impact_Plan_of_Actionloading = useSelector(state => state.popup.High_Impact_Plan_of_Actionloading)
  const High_Impact_Plan_of_Action = useSelector(state => state.popup.High_Impact_Plan_of_Action)
  /*  */
  ...
  useEffect(()=>{
    if(selectedDimension && visible){
      if (Tile == 1){
        dispatch(fetchHighImpact_Initiative_popup_dimension_1({
          "elasticQueryName": "",
          "filters": [],
          "dynamicColumns": [
            {
        ...
        dispatch(fetchHighImpact_Initiative_popup_dimension_2({
            "elasticQueryName": "",
            "filters": [],
            "dynamicColumns": [
              {
        ...
        dispatch(fetchHighImpact_Initiative_popup_dimension_3({
            "elasticQueryName": "",
            "filters": [],
            "dynamicColumns": [
              {
  ...
  useEffect(()=>{
    if (visible){
      dispatch(fetchHigh_Impact_Plan_of_Action({
        "elasticQueryName": "",
          "filters": [],
          "dynamicColumns": [],
      }))
  ...
  useEffect(()=>{
    if(selectedDimension){
      if (Tile == 1){
        setactiveTileBarData(HighImpact_Initiative_popup_dimension_1)
      } else if (Tile == 2){
        setactiveTileBarData(HighImpact_Initiative_popup_dimension_2)
      } else if (Tile == 3){
        setactiveTileBarData(HighImpact_Initiative_popup_dimension_3)
      }
    }
  },[selectedDimension, Tile, HighImpact_Initiative_popup_dimension_1, HighImpact_Initiative_popup_dimension_2, HighImpact_Initiative_popup_dimension_3])
...
const events = useMemo(()=>{
  let array_len = High_Impact_Plan_of_Action.length
  let Initiative = {}
  let data = High_Impact_Plan_of_Action.reduce((acc,item,index)=>{
  ...
  for (let range in data){
    ...
    for (let date in data[range]){
      let Boards = [...new Set(Initiative?.[date])]?.filter(item => item?.Board != "NULL" && item?.Board)
      let value = {
        ...
        <Dialog header="Header"
          ...
          <div>
            <div className="bg_bg_brand_neutral_100 flex max-md:flex-wrap items-center justify-between 2xl:gap-[1.563vw] gap-5 2xl:py-[0.677vw] py-[13px] 2xl:px-[1.25vw] px-5">
              <div>
                <Image src={'/images/diverse-staff-logo.png'} width={96} height={66} alt="Equitable Opportunities & Outcome" className="2xl:w-[5.469vw]
                2xl:h-[3.646vw]" />
              </div>
              <div className="2xl:w-[16.094vw] w-[300px]">
                <div className="white_text_color text-xs 2xl:text-[0.625vw] font-light leading-3 2xl:leading-[0.833vw]">STAFF RETENTION</div>
                <div className="2xl:text-[0.938vw] text-sm white_text_color leading-5 2xl:leading-[1.146vw] font-light">High Impact, Diverse Staff</div>
              </div>
              <div className="white_text_color text-xs opacity-60 2xl:text-[0.729vw] font-light leading-4 2xl:leading-[0.885vw]">
                {/* <p>Each initiative represents the initiative KPS is tracking throughout the implementation of the strategic plan. Indicators correspond to one of the four strategic plan goals. For more informatoin </p> */}
            ...
            <div className="brand-neutral-50 flex items-center 2xl:gap-[0.833vw] gap-4   2xl:h-[3.125vw] md:h-[65px] xl:h-[55px]">
              ...
              <div className="text-[16px] xl:text-[0.885vw]  font-normal leading-6 2xl:leading-[1.219vw] flex items-center 2xl:gap-[0.417vw] gap-1">
                ...
                {/* <div className="white_text_color 2xl:text-[0.938vw] text-base font-light leading-6 2xl:leading-[1.25vw]"><p>Establish partnerships with postsecondary institutions, government, and community-based organizations to create career pathways for diverse candidates</p></div> */}
                <div className="white_text_color  text-[16px] xl:text-[16px] 2xl:text-[0.885vw] text-base font-light leading-6 2xl:leading-[1.25vw]"><p>{TopTiledata?.TITLE || ""}</p></div>
              </div>
            </div>
              ...
              <div className="bg_blue_color_900  p-[18px] xl:p-[18px] 3xl:p-[0.938vw] 2xl:space-y-[1.25vw] space-y-[20px]">
                ...
                <div className="border border_color_interface-stroke-default 2xl:pt-[0.833vw] pt-3 2xl:px-[0.833vw]  px-3 2xl:space-y-[1.25vw] space-y-[20px] rounded-[0.417vw]">
                  ...
                  <div className="w-full overflow-auto">
                    <LoaderContainer loading={High_Impact_Plan_of_Actionloading}  width={"100%"} height={'10rem'}>
                ...
                {/* <div>
                  <ChartWrapper
                    ...
                    formatFileName={'High Impact Diverse Staff Data'}
                    ExportIcon={true}
                      ...
                      <>
                        <div>
                          <div className="bg-[#1F2A37] 2xl:rounded-[0.313vw] flex items-center rounded 2xl:p-[0.417vw] p-1.5 2xl:mt-[0.417vw] mt-1.5">
                            <div className="text-[#FFFFFF] text-xs 2xl:text-[0.625vw] font-normal leading-4 2xl:leading-[0.833vw] mr-2">Choose Dimension:</div>
                            <div>
                              <Dropdown value={selectedDimension} onChange={(e) => setselectedDimension(e.value)} options={Dimensions} optionLabel="name"
                                placeholder="Select a Dimensions" className="w-full md:w-[309px] 2xl:w-[18.229vw] 2xl:h-[1.51vw] h-[24px] custom_Dropdown" panelClassName="custom_Dropdownpanel" />
                            </div>
                          </div>
                          ...
                          <div className="text-[#FFFFFF] text-xs 2xl:text-[0.625vw] font-normal leading-4 2xl:leading-[0.833vw] mr-2">
