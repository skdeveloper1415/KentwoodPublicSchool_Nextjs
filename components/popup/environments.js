...
import { fetchWholeChild_Initiative_popup_dimension_1, fetchWholeChild_Initiative_popup_dimension_2,
  fetchWhole_Child_Environment_Plan_of_Actions
 } from "../../redux/slices/popup";
import { FormatNum } from "../utils";
import LoaderContainer from "../LoaderContainer";

export default function WholeChildEnvironment(props) {
  ...
  /* Redux States */
  const WholeChild_Initiative_popup_dimension_1loading = useSelector(state => state.popup.WholeChild_Initiative_popup_dimension_1loading)
  const WholeChild_Initiative_popup_dimension_1 = useSelector(state => state.popup.WholeChild_Initiative_popup_dimension_1)

  const WholeChild_Initiative_popup_dimension_2loading = useSelector(state => state.popup.WholeChild_Initiative_popup_dimension_2loading)
  const WholeChild_Initiative_popup_dimension_2 = useSelector(state => state.popup.WholeChild_Initiative_popup_dimension_2)

  const Whole_Child_Environment_Plan_of_Actionsloading = useSelector(state => state.popup.Whole_Child_Environment_Plan_of_Actionsloading)
  const Whole_Child_Environment_Plan_of_Actions = useSelector(state => state.popup.Whole_Child_Environment_Plan_of_Actions)
  /*  */
  ...
  useEffect(()=>{
    if(selectedDimension && visible){
      if (Tile == 1){
        dispatch(fetchWholeChild_Initiative_popup_dimension_1({
          "elasticQueryName": "",
          "filters": [],
          "dynamicColumns": [
            {
        ...
        dispatch(fetchWholeChild_Initiative_popup_dimension_2({
            "elasticQueryName": "",
            "filters": [],
            "dynamicColumns": [
              {
  ...
  useEffect(()=>{
    if (visible){
      dispatch(fetchWhole_Child_Environment_Plan_of_Actions({
        "elasticQueryName": "",
          "filters": [],
          "dynamicColumns": [],
      }))
  ...
  useEffect(()=>{
    if(selectedDimension){
      if (Tile == 1){
        setactiveTileBarData(WholeChild_Initiative_popup_dimension_1)
      } else if (Tile == 2){
        setactiveTileBarData(WholeChild_Initiative_popup_dimension_2)
      }
    }
  },[selectedDimension, Tile, WholeChild_Initiative_popup_dimension_1, WholeChild_Initiative_popup_dimension_2])
...
const events = useMemo(()=>{
  let array_len = Whole_Child_Environment_Plan_of_Actions.length
  let Initiative = {}
  let data = Whole_Child_Environment_Plan_of_Actions.reduce((acc,item,index)=>{
        ...
        <Dialog header="Header"
          ...
          <div>
            <div className="bg_bg_brand_neutral_100 grid grid-cols-1 md:grid-cols-12 flex items-center justify-between 2xl:gap-[1.563vw] gap-[20px] 3xl:gap-[1.25vw] 2xl:py-[0.677vw] py-[13px] 2xl:px-[1.25vw] px-5">
              <div className="grid grid-cols-1 md:grid-cols-3 items-center col-span-1 md:col-span-4">
                <div className="col-span-1 md:col-span-1"><Image src={'/images/wce.png'} width={102} height={70} alt="Equitable Opportunities & Outcome" className="2xl:w-[5.313vw] 2xl:h-[3.646vw]" /></div>
              ...
              <div className="col-span-1 md:col-span-8 white_text_color text-xs opacity-60 2xl:text-[0.729vw] font-normal leading-4 2xl:leading-[0.885vw]">
              ...
              <div className="bg_blue_color_900  p-[18px] xl:p-[18px] 3xl:p-[0.938vw] 2xl:space-y-[1.25vw] space-y-[20px]">
                ...
                <div className="border border_color_interface-stroke-default 2xl:pt-[0.833vw] pt-3 2xl:px-[0.833vw]  px-3 2xl:space-y-[1.25vw] space-y-[20px] rounded-[0.417vw]">
                  ...
                  <div className="w-full overflow-auto">
                    <LoaderContainer loading={Whole_Child_Environment_Plan_of_Actionsloading}  width={"100%"} height={'10rem'}>
                ...
                {/* <div>
                  <ChartWrapper
                    ...
                    formatFileName={'Whole-Child Environments Data'}
                    ExportIcon={true}
